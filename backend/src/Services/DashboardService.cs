...
public class DashboardService
{
    ...
    public async Task<ComprehensiveDashboardDto> GetComprehensiveDashboard()
    {
        ...
        try
        {
            ...
            {
                Overview = results[0] as DashboardStatsDto,
                Students = results[1] as StudentStatsDto,
                Teachers = results[2] as TeacherStatsDto,
                Courses = results[3] as CourseStatsDto,
                Grades = results[4] as GradeStatsDto,
                Departments = results[5] as DepartmentStatsDto,
                Activity = results[6] as ActivityStatsDto,
                Health = results[7] as SystemHealthDto,
    ...
    public async Task<CourseStatsDto> GetCourseStats()
    {
        ...
        var topEnrolledCourses = await _context.StudentCourses
            .Join(_context.Courses, sc => sc.CourseId, c => c.Id, (sc, c) => new { sc, c })
            .Join(_context.Departments, x => x.c.DepartmentId, d => d.Id, (x, d) => new { x.sc, x.c, x.d })
            .Join(_context.Teachers, x => x.c.TeacherId, t => t.Id, (x, t) => new { x, x.sc, x.c, x.d, x.t })
            .GroupBy(x => x.c.Id)
        ...
        var leastEnrolledCourses = await _context.StudentCourses
            .Join(_context.Courses, sc => sc.CourseId, c => c.Id, (sc, c) => new { sc, c })
            .Join(_context.Departments, x => x.c.DepartmentId, d => d.Id, (x, d) => new { x.sc, x.c, x.d })
            .Join(_context.Teachers, x => x.c.TeacherId, t => t.Id, (x, x, x.sc, x.c, x.d, x.t })
            .GroupBy(x => x.c.Id)
            .Select(g => new CourseEnrollmentDto
            {
                CourseId = g.Key,
