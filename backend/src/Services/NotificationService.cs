...
public class NotificationService : INotificationService
{
    ...
    public async Task<Dictionary<string, int>> GetNotificationStatsAsync()
    {
        var stats = await _context.Notifications
            .GroupBy(n => n.Type)
            .ToDictionaryAsync(g => g.Key, g.Count());
    ...
    public async Task NotifyGradeAddedAsync(int studentId, string courseName, double grade)
    {
        ...
        await CreateNotificationAsync(notificationDto, student.User.Id);
    }
    ...
    public async Task NotifyCourseUpdatedAsync(string courseName, string changeDescription)
    {
        var students = await _context.StudentCourses
            ...
            .Join(_context.Students, x => x.sc.StudentId, s => s.Id, (x, x, s) => new { x.sc, x.c, s })
            .Join(_context.Users, x => x.s.User, u => u.Id, (x, x, x, u) => new { x, x, x, x })
            .Select(x => new { StudentId = x.s.Id, UserId = x.u.Id, StudentName = x.s.FullName, CourseName = x.c.Name, Email = x.u.Email })
            .ToListAsync();
